...
class SharedReplayBuffer(object):
    ...
    def feed_forward_generator_pinn(self, advantages, num_mini_batch=None, mini_batch_size=None):
        """
        Yield minibatches that keep all agents from the same env step together.

        PINN actors build an inter-agent graph from each env step, so a minibatch
        must be sampled by (time, rollout_thread) groups instead of individual
        agents. The yielded tensors are still flattened back to [B * agents, dim]
        to match the MAPPO policy interface.
        """
        return self.feed_forward_generator_transformer(
